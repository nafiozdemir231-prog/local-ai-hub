    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 dark:from-gray-900 dark:to-gray-950">
      <div className="container mx-auto px-4 py-12">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-3">Multimodal</h1>
                ...
                <tr key={model.name} className="transition-all duration-150 hover:bg-indigo-50/50">
                  ...
                  <td className="py-4 px-5">
                    <div className="flex items-center gap-2">
                      <Link href={model.websiteUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:underline transition-colors">
                        {model.name}
                      </Link>
                      <ExternalLink className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />
                    </div>
                  </td>
                  <td className="py-4 px-5 text-sm text-gray-700 dark:text-gray-300 font-mono">{model.os}</td>
                  <td className="py-4 px-5 text-sm text-gray-600 dark:text-gray-400">{model.platform}</td>
                  <td className="py-4 px-5 text-sm text-gray-600 dark:text-gray-400">{model.supported}</td>
                  <td className="py-4 px-5 text-sm text-gray-500 dark:text-gray-400 max-w-xs">{model.explanation}</td>
                </tr>
              ))}
            </tbody>
          </table>
